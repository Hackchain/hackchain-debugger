...
function Debugger(title, data) {
  ...
  this.data = {
    ...
    output: Array.isArray(data.output) ?
        TX.Script.compileTextArray(data.output).opcodes : data.output,
    input: Array.isArray(data.input) ?
        TX.Script.compileTextArray(data.input).opcodes : data.input
  };
...
Debugger.prototype.updateThread = function updateThread(box, bar, thread) {
  ...
  lines = lines.map((line, i) => {
    return `${addr(Math.max(0, (start >> 1) + i))}: ${line}`;
  });
